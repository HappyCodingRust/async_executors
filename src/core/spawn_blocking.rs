...
pub trait SpawnBlocking<T: Send> {
    /// spawn a blocking function
    fn spawn_blocking_obj(&self, func: Box<dyn FnOnce() -> T + Send>) -> Result<JoinHandle<T>, SpawnError>;
}

/// Spawn a blocking task, maybe in a thread pool(tokio), or in current thread and spawns a new thread(std-async)
pub trait SpawnBlockingExt<T: Send>: SpawnBlocking<T> {
    /// spawn a blocking function
    fn spawn_blocking(&self, func: impl FnOnce() -> T + Send + 'static) -> Result<JoinHandle<T>, SpawnError> {
...
pub trait SpawnBlockingStatic {
    /// spawn a blocking function
    fn spawn_blocking<T>(func: impl FnOnce() -> T) -> Result<JoinHandle<T>, SpawnError>;
}